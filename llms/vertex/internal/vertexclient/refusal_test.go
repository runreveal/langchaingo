...
// guardrailBody is the shape a Claude model on Vertex returns when a request trips
// a safety guardrail: an empty content array, stop_reason "refusal", and the reason
// in stop_details.
const guardrailBody = `{
  "type": "message",
  "role": "assistant",
...
func TestAnthropicRefusalParsesStopDetails(t *testing.T) {
	...
	require.Equal(t, anthropicStopReasonRefusal, out.StopReason)
...
func TestAnthropicRefusalChoice(t *testing.T) {
	t.Run("refusal with explanation", func(t *testing.T) {
		...
		choice := anthropicRefusalChoice(anthropicStopReasonRefusal, details, 87, 0)
		require.NotNil(t, choice)
		require.Equal(t, "Blocked.", choice.Content)
		require.Equal(t, anthropicStopReasonRefusal, choice.StopReason)
	...
	t.Run("refusal without explanation falls back to a non-empty message", func(t *testing.T) {
		choice := anthropicRefusalChoice(anthropicStopReasonRefusal, nil, 10, 0)
		require.NotNil(t, choice)
		require.NotEmpty(t, choice.Content)
	})

	t.Run("non-refusal empty response is not synthesized", func(t *testing.T) {
		require.Nil(t, anthropicRefusalChoice(anthropicStopReasonToolUse, nil, 10, 0))
		require.Nil(t, anthropicRefusalChoice(anthropicStopReasonEndTurn, nil, 10, 0))
	})
}
